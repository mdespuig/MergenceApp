...
from django.views.decorators.http import require_POST
from .models import EmergencyReport, PartnerApplication, Hotline, TimelineUpdates, Profile, Message
import json
...
def messenger(request):
    ...
    return render(request, 'messenger.html', {'users': users})
...
def private_chat(request, username):
    ...
    room_name = "_".join(sorted([current_user.username, chat_partner.username]))

    messages = Message.objects.filter(
        Q(sender=current_user, receiver=chat_partner) |
        Q(sender=chat_partner, receiver=current_user)
    ).order_by('timestamp')
